...
void Pi0_mass_calculation(Double_t Targetname, int RunNo, int Sgemin, int Segmax, Double_t angle, Double_t beamEne, Double_t* result){
    ...
    Double_t NofCharge, Target_Type, BeamCurrent;
    std::string line;
    ...
    if(Target_Type>1.&&Target_Type<3.){
        // std::cout<<"Target type is LH2 or LD2."<<std::endl;
        if(Target_Type>1.&&Target_Type<2.){
            // if(BeamCurrent<25.||BeamCurrent>30.){ // Kin_x60_3 LD2_20uA LH2_30uA
            // if(BeamCurrent<10.||BeamCurrent>15.){ // Kin_x36_3 LD2_15uA LH2_15uA
            // if(BeamCurrent<20.||BeamCurrent>25.){ // Kin_x36_5 LD2_15uA LH2_25uA
            if(BeamCurrent<30.||BeamCurrent>35.){ // Kin_x50_4 LD2_15uA LH2_35uA
                result[0] = -1.;
                return;
            }
        }
        if(Target_Type>2.&&Target_Type<3.){
            // if(BeamCurrent<15.||BeamCurrent>20.){ // Kin_x60_3 LD2_20uA LH2_30uA
            // if(BeamCurrent<10.||BeamCurrent>15.){ // Kin_x36_3 LD2_15uA LH2_15uA
            // if(BeamCurrent<10.||BeamCurrent>15.){ // Kin_x36_5 LD2_15uA LH2_25uA
            if(BeamCurrent<10.||BeamCurrent>15.){ // Kin_x50_4 LD2_15uA LH2_35uA
                result[0] = -1.;
                return;
            }
        }
    ...
    TString File_Path = "/volatile/hallc/nps/cmorean/ROOTfiles/COIN/SKIM/";
    ...
    // for(Long64_t evt= 0; evt < 1000; evt++){
        ...
        for(int i=0; i<nclust;i++){
            for(int j=i+1; j<nclust;j++){
                ...
                if(clusE[i]<1.0||clusE[j]<1.0) continue;
...
void pi0_charge(){
    ...
    // "/volatile/hallc/nps/cmorean/ROOTfiles/COIN/SKIM/"
    // "/volatile/hallc/nps/cmorean/REPORT_OUTPUT/COIN/SKIM/"
    ...
    outputFile.open(Form("./pi0_charge/%d_%d.txt",RunBegin,RunEnd));
    ...
    for(int RunNo=RunBegin;RunNo<=RunEnd;RunNo++){
        ...
        outputFile<<RunNo<<"\t"<<result[0]<<"\t"<<result[1]<<"\t"<<result[2]<<"\t"<<result[3]<<std::endl;
