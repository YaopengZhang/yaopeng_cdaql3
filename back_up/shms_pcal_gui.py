...
def main():
    ...
    while True:
        ...
        zvals2 = getScalars()
        for ii in range(len(zvals)):
            hh.SetBinContent(xax.FindBin(XVALS[ii]+1),yax.FindBin(16-YVALS[ii]),(zvals2[ii]-zvals[ii])/10.)
            hi.SetBinContent(xax.FindBin(XVALS[ii]+1),yax.FindBin(16-YVALS[ii]),(zvals2[ii]-zvals[ii])/10.)

    
        for xx in [ttime,ttM,tt1,ttLM,ttRM]: xx.Clear()
        [left,right,maximum]=calcRates(zvals,zvals2)
        ttLM.AddText('Left total: %.1f kHz'%(left/1000.))
        ttRM.AddText('Right total: %.1f kHz'%(right/1000.))
